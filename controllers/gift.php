<?php
Class Controller_Gift Extends Controller_Base 
{
  ...
  public function index() 
  {
    ...
    $template = $this->_initTemplate($mygift_title, '');
    
    $template->setFile('templates/gift.phtml');

    $template->set('row_user', $row_user);

    $query_user_subscription = "SELECT t1.id, t2.name, t2.nickname, t2.avatar_user, t2.date_birth, t1.email_recipient_subscription, t1.locked, t1.date_subscription FROM `subscription_users` as t1 JOIN `registered_users` as t2 on t1.email_recipient_subscription = t2.email WHERE email_sender_subscription='$user_email' ORDER BY id DESC";
